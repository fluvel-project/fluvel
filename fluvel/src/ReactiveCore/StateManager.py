...
from typing import Pattern, Any, Dict, Tuple, Callable
import re

# Fluvel
from fluvel.src.ReactiveCore.Model import Model, ModelStore
...
class StateManager:
    ...
    of all :py:class:`"Model"` in the application.
    ...
    * ``text:@vm.username`` (**Level 2**: Explicit unidirectional binding 'Model -> View')
    * ``text:textChanged:@vm.username`` (**Level 3**: Explicit Bidirectional binding 'Model <-> View')
    * ``~text:textChanged:@vm.username`` (**Level 4**: Inverted unidirectional binding 'View -> Model')
    ...
    def bind(cls, widget: QWidget, bind_string: str) -> None:
        ...
        model = ModelStore.get_model(alias)

        filter_fn, template = cls.decode_formatter(parsed_binding)
        prop_name, signal_name, is_source_only = cls.decode_level(parsed_binding, widget)
        ...
        if signal_name:
            ...
            if key in model.__class__._computed_names:
                raise FluvelBindingError(
                    f"The computed state '{key}' of the model '@{alias}' is read-only. "
    ...
    @classmethod
    def decode_level(self, parsed_binding: Dict[str, str], widget: QWidget) -> Tuple[str, str, bool]:
        ...
        # Level 4 Binding
        if is_source_only := prop_name.startswith("~"):
            prop_name = prop_name.lstrip("~")

        return prop_name, signal_name, is_source_only
    ...
    def set_unidirectional_binding(
        ...
        :type model: :py:class:`~fluvel.src.ReactiveCore.Model.Model`
        ...
        :type filter_fn: :py:class:`~typing.Callable` | None
        ...
        if template:
            widget_base_type = type(widget.property(prop_name))

        def apply_filter(value: Any):
        ...
        def apply_format(new_value: Any, template: str | None) -> Any:

            match template:
                case None: return new_value
                case "%v": 
                    new_value = apply_filter(new_value)
                    return widget_base_type(new_value)
                case _: 
                    new_value = apply_filter(new_value)
                    return template.replace("%v", str(new_value))

        # model.key -> Calls __get__ of the descriptor -> Returns value
        initial_value = getattr(model, key)
        widget[prop_name] = apply_format(initial_value, template)

        # Reactive Update, the unique ‘model_changed’ signal for each model is used
        def update_widget(changed_key: str, new_value: Any):
            """Slot that updates the widget only if the key matches."""

            if changed_key == key:
                
                widget[prop_name] = apply_format(new_value, template)

        # We obtain the centralized signal (the only Qt signal that exists in Model)
        # model.get_signal(key) already returns model_changed, but it serves as a checkpoint
        # to validate that the key exists in the Model.
        model.qt_emitter.model_changed.connect(update_widget)
    ...
    def set_bidirectional_binding(
        ...
        .. note::
            ...
            using :py:meth:`~StateManager.set_unidirectional_binding`.
        ...
        try:
            ...
            raise FluvelStateError(
                f"The widget '{widget.__class__.__name__}' does not have a signal '{signal_name}'."
            )
        ...
        def update_model(*args):
            ...
            # We read the current value of the model (call the GET descriptor)
            current_model_value = getattr(model, key)

            if current_model_value != widget_value:
